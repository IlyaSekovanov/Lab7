...
void outvector(vector<int>& s, int &n) //Вывод вектора (Если нужно)
{
	for (int i = 0; i < n; ++i) 
	{
		cout << s[i] << endl;
...
void mergesort(vector<int>& s, size_t start, size_t end) //Сортировка слиянием (Илья)
{ 
	if (end - start < 2) 
	{
		return;
...
void heapsort(vector <int> &s, int &n) //Пирамидальная (Влад)
{
...
void vstavka(vector<int> &s, int &n) // сортировка вставками (Саша)
{
...
void shell(vector <int> &arr, int &n)  //Шелла (Никита Бархатов)
{
...
void qsort(vector<int> &s, int n) //быстрая сортировка (Никита Литовкин)
{ 
	int i = 0;
	int j = n - 1;
	int mid = s[n / 2];
